...
public class Percolation {
    ...
    private int count; // number of open sites, default initial value of 0
    private int lowestFullY;
    private int lowestFullX;
    ...
    //
    //??move up
    private linkSite(int row, int col, int locationIndex) {
        int newSite = toL(row, col);
        
        if (locationIndex == 1) int adjacentSite = toL(row
    }
    ...
    public void open(int row, int col) {
        ...
        if (grid[row][col] == false) {
            ...
            //check if lowest full site has changed
            //????
            
            
            if (isFull(row, col)) {
                //run through connections to find in bottom row, if found, perc
                
                
                
                //check if bottom row if connect
    ...
    public boolean isFull(int row, int col) {
        ...
        int linearIndex = toL(row, col);
        
        for (int i=0; i<N; i++) {
            if (sites.connected(i, linearIndex)) 
                return true;
        }
